...
Full context: `docs/SPEC.md` (canonical spec), `docs/DECISIONS.md` (why, not just what), `docs/qa/audits/PRODUCT-2026-07-13.md` (current health, honestly scored), `docs/qa/RELAY_GAP.md` (what a product-expansion brief asked for vs what actually exists, with the gap register and the two reversal proposals).
...
│             findings. Six dated audits, one of them a retired product audit.
...
**Release decision: NO-GO for general release · GO for a supervised pilot** — two churches, named operators, every service watched by somebody who can take the wall back by hand. Taken 2026-08-31 (`docs/qa/RELAY_GAP.md` §24) and re-verified on 2026-09-03 against a packaged binary. Relay has now run **three live services**. 2026-08-30 (`docs/qa/audits/FIELD-2026-08-30.md`): Stage F11 answered, no drift across 49.5 minutes, five of six auto-fires correct on `ggml-large-v3-turbo` — **and one wrong verse reached a congregation** (root-caused and fixed, `detection::anchor_for_bare_verses`). 2026-09-06, two services (`docs/qa/audits/FIELD-2026-09-06.md`): **three of three correct on `turbo`, then five of nine on `ggml-base` after the model was switched mid-service — four wrong verses, a 44% wrong-verse rate against SPEC's 5% bar.** Nothing reached a congregation-facing screen that morning (console program pane only). One of the four is fixed (F-8, RG-115: a chapter the window stated out loud was answered from memory). The other three are the model, and **the sensitivity dial is measurably not the lever** — its most cautious setting still keeps a wrong verse at 0.95 and discards a correct one (audit §2). **A pilot must pin the model, and nothing currently does.** Word error rate is still unmeasured, but there is now 85.5 minutes of real church audio to measure it against (RG-116). Word error rate is still unmeasured in every language, **neither platform has a code-signing certificate** (`gh secret list` holds zero of the fourteen; every release went out unsigned on the pre-release path — RG-73), the auto-updater's channel is **live** as of 2026-09-05 (RG-83 and RG-114 closed; `npm run updater:check` reports `2 update endpoints live` serving 0.2.0-2 — it was 404 for months and that clause sat here after the fix landed), and nobody but the author has run a service. Rule 17's trap is no longer merely ahead: the packaged build has now been signed under the hardened runtime with the microphone entitlement intact, booted, and probed, so the failure is reproducible locally without a certificate.
...
**Sixteen of the forty-one name the instrument that holds them** — 1, 3, 10, 15, 16, 17, 24, 25, 26, 32, 33, 35, 36, 38, 40 and 41; search for *"Pinned by"* and count, rather than trusting this sentence, which has been wrong before (it said ten while fifteen were pinned, and under-reported the safety coverage it exists to describe). The rest rest on discipline and review, which is the honest state: most are judgement (*never hold a `Mutex` across `emit`*) and cannot be a test. **A rule with no named instrument is not a weaker rule; it is one where you are the instrument.**

**Seven are pinned by `hardrules.test.js`** — 1 (`tick()` in a reactive block), 3 (the 16 MB STT stack), 9 (no hand-rolled `$HOME` app-data path), 24 (the fire path stays generic over `tauri::Runtime`), 33 (no unbounded queue anywhere on the audio path), 36 (`broadcast_content` has exactly one caller, inside `broadcast_with_clock`) and 41 (no native `confirm()`/`alert()`/`prompt()`). Rule 26 is pinned by `ipc.test.js`. The other eight named instruments live beside the code they guard — `router`, `panic.test.js`, `shortcuts.test.js`, `models::config_boots`, `db::ensure_service_plans_is_retryable`, `stt::adaptive_cadence`, `updatechannel.test.js`, `e2e::r2_…`. **The remaining twenty-five are judgement and deliberately have no test** — one that guessed would fail on legitimate code, get weakened, and take the real ones with it. Reproduce the split: `grep -c 'Pinned by' CLAUDE.md` counts the claims; opening `src/lib/hardrules.test.js` counts what that one file actually holds.
...
16. **`Esc` must not clear the screens while any dialog is open.** `shortcuts.js` checks for a mounted `[role="dialog"]`. Dismissing a help overlay or an arrangement picker is not a live action — it used to wipe the wall as a side-effect. **Pinned by `shortcuts.test.js` — *"Escape inside ANY open dialog does not clear the screens"*.**
...
- Tauri events: **19 of them**, and the list lives in `docs/ARCHITECTURE.md` §6, not here — the copy that used to sit on this line was missing five (`audio://quality`, `channel://retemplate`, `output://error`, `rehearsal://changed`, `stt://language_unstable`), and one of those is an error path. Reproduce the set with `grep -rhoE '"[a-z_]+://[a-z_]+"' src-tauri/src/*.rs | sort -u`; `ipc.test.js` holds the contract in both directions.
