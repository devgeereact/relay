...
**Ten of the forty-one now name the instrument that holds them** — search for *"Pinned by"*. The rest rest on discipline and review, which is the honest state: most are judgement (*never hold a `Mutex` across `emit`*) and cannot be a test. **A rule with no named instrument is not a weaker rule; it is one where you are the instrument.**
