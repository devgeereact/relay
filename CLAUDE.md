...
│       ├── channels.rs     — output render targets: native window + kiosk WS hub
│       └── db.rs           — SQLite: full-KJV import, templates, channels, service history, migrations
├── src/                    — Svelte frontend (console + views + TemplateRender + capture store)
...
- **This machine cannot screenshot the app** (Chrome ext account mismatch + no Screen Recording perm). Verify via `cargo test`, `vite build`, backend stdout, and — for the webview — a heartbeat forwarded to the backend log. (Global memory `relay-no-screenshot-path`.)
