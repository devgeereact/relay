<script>
  ...
  //
  // The prototype's Live workspace opens with a 206px rail: Bible / Songs, a
  // search field, and the books with their chapter counts. Relay's Live started
  // at the Preview monitor, so the ONLY way to reach a verse nobody had planned
  // for was to type it into the manual box and fire it blind. That is the
  // preacher-goes-off-script case — the entire reason this product exists — and
  // it had no browsing surface at all.
  //
  // THE ONE RULE THIS FILE KEEPS: nothing here reaches a screen. A search, a
  // book, a chapter, a song — every press STAGES into the slide grid, and the
  // grid is where a press becomes a take (and reports its own outcome, per
  // CLAUDE.md rules 15 and 18). A rail that fired would make a mistyped search
  // a congregation-facing event.
  //
  ...
  import { listBooks, listSongs, searchScripture, readErrors } from './stores/capture.js';
  ...
  /** Stage one song in the slide grid. Never fires. */
  export let onSong = () => {};
  ...
  // Songs filter in memory — the list is small, and `searchSongs` would give the
  // rail a second, differently-ordered answer from the Library's.
  $: songRows = songs.filter((s) => {
    const needle = q.trim().toLowerCase();
    if (!needle) return true;
    return `${s.title} ${s.author}`.toLowerCase().includes(needle);
  });
...
<aside class="lrail" aria-label="Scripture and songs">
  <div class="lr-head">
    <div class="lr-seg" role="group" aria-label="Browse Bible or Songs">
      <button class:on={tab === 'bible'} on:click={() => pickTab('bible')}>Bible</button>
      <button class:on={tab === 'songs'} on:click={() => pickTab('songs')}>Songs</button>
    </div>
  </div>
  ...
  <div class="lr-body">
    {#if tab === 'bible'}
      {#if hitsFor}
        <!-- A search STAGES the chapter it found. It does not fire: the grid
             below is where a press becomes a take. -->
        <p class="lr-cap">{hits.length} found · opens the chapter in the grid</p>
        {#each hits as h (h.id)}
          <button class="lr-row" {disabled} on:click={() => onChapter(h.book, h.chapter, h.reference)}>
      ...
      {/if}
    {:else}
      {#each songRows as s (s.id)}
        <button class="lr-row" {disabled} on:click={() => onSong(s)}>
          <span class="lr-i" aria-hidden="true">♪</span>
      ...
      {:else}
        ...
        {:else if $readErrors.listSongs}
          <ErrorState compact error={$readErrors.listSongs} onRetry={() => { songsLoaded = false; loadSongs(); }} />
        {:else}
          <EmptyState message={q.trim() ? 'No song matches.' : 'No songs yet — add them in the Library.'} />
        {/if}
      {/each}
    {/if}
  </div>
...
<style>
  ...
  .lr-seg { display: flex; border: 1px solid var(--v-line2); border-radius: var(--v-r-sm); overflow: hidden; }
  .lr-seg button {
    flex: 1; padding: 4px 0; border: 0; cursor: pointer; background: var(--v-surf);
    font-family: var(--f-body); font-size: 11.5px; font-weight: 600; color: var(--v-dim);
  }
  .lr-seg button.on { background: var(--v-sel-soft); color: var(--v-sel); }
  .lr-seg button:not(.on):hover { color: var(--v-txt); }
  ...
  .lr-q {
    ...
    color: var(--v-txt); font-family: var(--f-body); font-size: 11.5px;
  }
  ...
  .lr-i { flex: 0 0 auto; width: 12px; text-align: center; font-size: 9px; color: var(--v-dim); }
  .lr-n { flex: 1; min-width: 0; font-size: 11.5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  ...
  .lr-k { flex: 0 0 auto; font-size: 9px; color: var(--v-dim); }
