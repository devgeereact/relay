...
// one hidden inside `+N more` (RG-162, RG-163, driven on 2026-09-17 against the
// real backend).
//
...
// ─────────────────────────────────────────────────────────────────────────────
// RG-162 · THE WARNING THRESHOLD REACHES THE RAIL
// ─────────────────────────────────────────────────────────────────────────────
describe('a cue’s clock carries the warning window the operator chose', () => {
  ...
  itMounted('a cue carries no finished message, and the rail counts up rather than inventing one', async () => {
    ...
    // message text is wave 3 Track E. RG-162 records it.
    //
    // THE EXPECTED FIGURE CHANGED IN THE CONSOLIDATION MERGE, and the claim did not.
    // This asserted `0:00` on the branch it was written on, because wave 4's rail
    // stopped at zero. Wave 3's counts up, and the merge took wave 3's: `--tch`
    // budgets the column from the widest rendered string, so prose in that slot
    // sizes every column to its length. `+0:00` is the same absence one tick later
    // -- Relay still composed no words -- and it is what a rail that keeps counting
    // shows at the instant it arrives.
    const at = 1_700_000_000_000;
    cues = [notice(21, 'Reading', { timer_minutes: 12 })];
    await runPlan();
    await take(0);
    ...
    const rows = await railAt(at);
    expect(rows[0].value, 'never an empty box, and never a message nobody wrote').toBe(
      '+0:00',
    );
    vi.useRealTimers();
  });
...
// ─────────────────────────────────────────────────────────────────────────────
// RG-163 · A CUE'S CLOCK ENDS WHEN THE CUE DOES
// ─────────────────────────────────────────────────────────────────────────────
