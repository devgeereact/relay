...
describe('the wiring, read off the source', () => {
  ...
  it('Live calls startTimer in exactly one place, inside the cue fire path', () => {
    // Four bugs in this repository have the shape "a rule kept on one door and
    // skipped on its twin". A second `startTimer` beside `stepLive`, beside the
    // grid press or beside the transport would be the fifth.
    expect([...live.matchAll(/await startTimer\(/g)]).toHaveLength(1);
