// DECISIONS §89 — A PANIC CONTROL SILENCES A ROOM; IT DOES NOT STOP THE CLOCKS.
//
...
// given by nobody, to the question §89 exists to settle.
//
// ── Why this is its own file ──────────────────────────────────────────────────
//
// `stagezones.test.js` owns what the six zones show; this owns what a PANIC
// CONTROL takes. The two halves of §89 have to be asserted together or either one
...
const VERSE = {
  ...
  // exception for, on a stated ground, long before anybody wrote §89 down.
...
describe('DECISIONS §89 · a word to the preacher comes down with the screens', () => {
  ...
  for (const kind of PANIC) {
    ...
    it(`\`${kind}\` does not stop the service clock`, async () => {
      ...
      // THE OTHER HALF OF §89, and the reason the alert's answer is a decision
