...
<script>
  ...
  // A second poll would also be a second answer to "did the read fail", and the
  // two bands would then disagree about a bridge that is down for both of them.
  async function loadProgrammeTimers() {
    try {
      ...
      if (dead) return;
      // The countdown band keeps its last good list for the same reason the
      // Stage band does, and prints the same reason beside it.
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // THE SCREEN COUNTDOWN (operator instruction, 2026-09-20)
  // ══════════════════════════════════════════════════════════════════════════
  //
...
<style>
  ...
  @media (prefers-reduced-motion: no-preference){
    ...
    /* THE COUNTDOWN'S LAST-MINUTE BLINK, in this block rather than in one of its
       own for the reason written beside `@keyframes cdwarn`. Steps, not a fade,
       and only where motion is welcome: the blink exists to catch an eye that is
       not looking at it, and a viewer who asked for no motion still gets the RED,
       which is the information. `opacity` reflows nothing, so it belongs to the
       same three properties this block is restricted to. */
