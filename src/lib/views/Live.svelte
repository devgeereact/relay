...
<script>
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // THE SCREEN COUNTDOWN IS NOT ON THIS SURFACE (operator instruction, 2026-09-20)
  // ══════════════════════════════════════════════════════════════════════════
  //
  // It arrived here that morning, out of Quick tools, and left the console that
  // evening. About 420 lines went with it across the script, the markup and the
  // stylesheet: the hh:mm:ss fields, the clock-time target, the format select,
  // the figure and its three states, the six transport controls, the screen
  // picker, the reach line and the way back. `lib/countdown.js` is untouched and
  // so is every backend command it called.
  //
  // WHERE THE CAPABILITY IS: `ServicePlanner.svelte::addCountdownCue` builds a
  // countdown as a plan cue aimed at named screens, which is the pre-service
  // tool the earlier instruction described.
  //
  // WHAT IS GENUINELY GONE, recorded here rather than discovered in a service:
  // there is no live transport on any surface now. A countdown already on a wall
  // cannot be held, reset, nudged by a minute or put back. Restoring that means
  // giving it a surface; a comment cannot restore it.
...
<div class="con" class:fullscreen>
  ...
  <!-- ══════ THE SCREEN COUNTDOWN IS NOT ON THIS SURFACE ═══════════════════
       Removed on the operator's instruction, 2026-09-20, the same day it was
       moved here out of Quick tools. It was a band of its own under the Stage
       Timer carrying the length fields, a clock-time target, the format, the
       figure, the six transport controls, the screen picker and the way back.

       WHERE THE CAPABILITY LIVES NOW: `ServicePlanner.svelte::addCountdownCue`
       builds a countdown as a plan CUE, aimed at the screens it names, which is
       the half of the earlier instruction that called it a pre-service tool for
       the service going online. A cue fires from the plan like any other.

       WHAT WENT WITH IT, SAID PLAINLY RATHER THAN DISCOVERED IN A SERVICE: with
       no band there is no live transport, so a countdown already on a wall can
       no longer be held, re-aimed, nudged by a minute or put back from here. The
       backend commands still exist and are still registered. If that control is
       wanted again it belongs on a surface, not in a comment. -->
