...
it('R6-3: every kiosk client has a DECISION about every kind the hub publishes', () => {
  ...
  const EXPECTED = {
    'src/Output.svelte': {
      ...
      channel_looks: true,
      // WHICH KINDS THIS SCREEN SHOWS AT ALL (DECISIONS §98). This page is the
      // party that acts on it, for the third time on the same argument: the hub
      // broadcasts to everybody and records nothing about who connected (§35).
      //
      // It is the one frame in this list that can stop a fire painting here, so
      // the thing worth saying is what it CANNOT stop: it is ANDed with
      // `templateShows` at one helper and is never consulted for `clear` or
      // `black`. `src/lib/channelshows.test.js` drives the real page and asserts
      // that a panic control reaches a screen configured out of the kind that is
      // on it.
      channel_shows: true,
    ...
    'src/Stage.svelte': {
      ...
      channel_looks: false,
      // WHAT A CONGREGATION SCREEN SHOWS — DELIBERATELY NOT, and this one is not
      // the "no TemplateRender" reason the rows around it give. It is narrower and
      // more important: this page is the preacher's, the five content kinds are
      // what a CONGREGATION sees, and the stage page renders none of them as
      // content — it paints the reading, the programme and a word meant for one
      // person. An operator who takes the countdown off the lobby TV has said
      // nothing about what the platform is reading, and a page that applied this
      // set would let a screen configuration silently blank the one screen a
      // preacher cannot look away from.
      channel_shows: false,
