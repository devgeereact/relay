...
| **T4b** | Content *read out of* the kiosk socket by a page nobody in the church opened deliberately | `:8031`, from any `http://` page on the church wifi — WebSockets are exempt from CORS | **MITIGATED 2026-09-05 (RG-108, DECISIONS §64).** The hub subscribed a client at accept, before any `hello`, so a page on a congregant's phone could receive the feed — and the feed carries `stage_note`, `next_reference` and `next_text`, the preacher's monitor. The handshake is now refused with a `403` unless the `Origin` is one Relay served (`:8032`, `:5032`, `tauri://localhost`) or absent. `RELAY_KIOSK_ANY_ORIGIN=1` restores the old behaviour for a church hosting its own kiosk page. **An origin header is a claim a browser makes honestly and a non-browser client can forge**, so this bounds the page-on-a-phone case, not an attacker with their own software on the LAN — which is T5's territory and is accepted by DECISIONS §35 |
