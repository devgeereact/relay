...
> | | Rev 3 said | Today |
> |---|---|---|
> | Tests | 250 Rust + 138 frontend | **584 Rust** run (28 ignored) + **747 frontend** (0 skipped) — re-measured 2026-08-29 |
> | `main.rs` | 2,922 lines / 101 commands | **5,288 lines / 134 commands** — re-measured 2026-08-29 |
> | Decision log | 25 decisions | **48** numbered (§18–§48), plus 28 earlier table rows |
...
| **Architecture** | **8 / 10** | ▲ +2 | The fire engine is now generic over `tauri::Runtime`, so the path that puts scripture on a wall can be driven **without a window** — which is the useful half of "split `main.rs`". Typed errors (`error.rs`) replace 88 × `Result<_, String>`, and **zero remain**. `main.rs` was 2,922 lines / 101 commands here and is **5,288 / 134** today — but it is no longer untestable, which was the actual problem. |
