...
| `docs/DECISIONS.md` | 42 decisions with reasoning, plus explicit non-goals | **Highest** for "why" |
...
Internally, the frontend↔core contract is **123 registered `#[tauri::command]`s**
...
`docs/DECISIONS.md` holds 42 numbered decisions with reasoning. The ones that constrain any
...
| Debt | State |
|---|---|
| `main.rs` — 4,799 lines / 123 commands | No longer a correctness issue (the fire engine is generic over `tauri::Runtime` and covered by `e2e.rs`); a readability complaint |
