...
| `docs/DECISIONS.md` | 48 decisions with reasoning, plus explicit non-goals | **Highest** for "why" |
...
Internally, the frontend↔core contract is **134 registered `#[tauri::command]`s**
...
`docs/DECISIONS.md` holds 48 numbered decisions with reasoning. The ones that constrain any
...
| Debt | State |
|---|---|
| `main.rs` — 5,288 lines / 134 commands | No longer a correctness issue (the fire engine is generic over `tauri::Runtime` and covered by `e2e.rs`); a readability complaint |
