...
| ⏳ RG-30 | **FIELD F-5** — every published latency figure is `ggml-base`. The model this church actually chose, `ggml-large-v3-turbo`, had never been measured on any machine | A church picks a model with no idea what it costs; the docs quote 139 ms and the room gives 687 ms | §2 of the field audit | Publish a per-model table from `stt::decode_cost`, and say which model each figure belongs to | — | P2 | S | `PERF-*.md` and `LANGUAGES.md` name the model beside every number |
---
