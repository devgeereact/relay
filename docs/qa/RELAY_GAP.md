...
| 37 | Accessibility mode (high visibility) | **EXISTS** | RG-18. High Visibility is a **look a template carries**, not a parallel renderer — so it inherits the one template engine and cannot drift from it (`legibility.test.js`). It shipped as a THEME; themes were folded into templates (DECISIONS §87) and its style is frozen in `src-tauri/data/legacy_themes.json` and inlined into any template that pinned it, so a church that chose it still has it — **but until Track D seeds it as a template family there is no high-contrast look on the shelf for a church that did not.** Contrast is computed exactly over a solid background and reported as **"cannot be checked"** over a picture, never guessed. The console side was already served: `sr-only` live regions, `role="meter"`, `trapFocus`, reduced motion | Nothing in the product. The same Stage B caveat as §36 |
