...
| ⏳ RG-117 | **A runtime audio-stream error is a single line on stderr: nothing stops, nothing is told, and the debug recording is lost.** `audio.rs:592` is `let err_fn = \|e\| eprintln!("audio stream error: {e}");` — the whole handler. It does not set the stop flag, does not emit `audio://error`, and does not reach the operator. Observed 2026-09-06 when the desk feed was unplugged: *"The requested device is no longer available. For example, it has been unplugged."* The capture loop is `while !stop.load(…)` with a 100 ms `recv_timeout`, so a dead stream leaves it spinning on `Timeout => continue` for ever | **Three failures in one, and the operator sees none of them.** (1) The transcript simply stops mid-sermon with no banner, no degraded line and nothing in the UI — indistinguishable from a preacher who has gone quiet. (2) `RELAY_RECORD_WAV` writes after the loop exits, so a device drop means the recording is never written — the instrument for the one measurement this project most needs is destroyed by the failure it would most want to have captured. (3) CLAUDE.md rule 5 states *"device errors come back via the `audio://error` event"*; that is true of **start** errors and false of runtime ones, so the handbook overstates the coverage. This nearly bit today: the WAV sat unwritten in RAM for 26 minutes while the app looked alive, and only a deliberate Stop released it | `audits/FIELD-2026-09-06.md` F-10 and F-11; read of `audio::build_stream` and its one-line `err_fn`, 2026-09-06, after the unplug line appeared in a live log. The buffered-and-unwritten state was observed directly: `voiced` climbing 18377 → 18870 with **one** `RECORDING` line in the log and no `audio: wrote`, and RSS falling 1.06 GB → 484 MB as the OS compressed the still-live buffer rather than because anything was freed | **FIXED 2026-09-07, all three parts.** `note_stream_error` stores the device's reason and sets the stop flag, so the loop exits, `drop(stream)` runs, and the recording is written. **Order matters and is asserted**: the WAV lands on disk before the error is returned into `AudioEngine::start`'s `on_error`, so by the time the operator is told the microphone died, the audio that proves what happened is already saved. The first error wins, because cpal fires this repeatedly while a device tears down and the first message is the cause while the rest are consequences; and it never blocks, because it can run on the device's own real-time thread, so a contended lock loses the message and still sets the flag. **The operator is told in the shell, not on one tab.** `audio://error` already reached `capture.audioError`, and the only surface rendering it was Live — a volunteer setting up a template or reading Help would never have seen it. There is now a rose `role="alert"` bar in `App.svelte`, below the panic bar (a panic control that failed outranks a dead microphone) and never amber, which means ON AIR. Not a `degraded.js` row on purpose: that file's own rule is that a failure is not a degradation. **The separate truncation hazard is fixed too.** `free_recording_path` never writes over an existing file; the next free `-2`, `-3` … is taken and the write line prints where the audio actually went. Resolved at write time rather than at Start, which is what makes the previous segment visible to it. CLAUDE.md rule 5 said device errors come back via `audio://error`, which was true of start errors and false of runtime ones; it now says both and names the instruments. **Still NOT verified, and this is the honest gap**: nothing here has been run against a real device being unplugged. The unit tests pin the decision (`a_stream_error_stops_capture_and_keeps_its_reason`, `the_first_reason_is_the_one_kept`, `a_recording_never_lands_on_a_file_that_already_exists`) and `audioerror.test.js` pins the surface, but the callback firing at all is cpal's behaviour on a real teardown and that wants the two-process rig RG-113 also wants. The reason this was deferred once — it is the audio thread's shutdown path, changed on the day a service had just depended on it — is why the change is as small as it is and why the ordering is asserted rather than assumed. **The row stays open until an unplug has actually been done**, because everything above is a decision pinned by unit tests and none of it is evidence about cpal on a real teardown | RG-09 for the degraded surface it should use | **P1** | M | Unplug the input mid-capture and assert three things: the recording is on disk, the shell shows a degraded state, and the operator is told. None of the three holds today |
