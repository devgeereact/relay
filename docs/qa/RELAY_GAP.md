...
**113 entries. 107 closed, 1 withdrawn as wrong, 5 not closed** — RG-32 open, RG-41 and RG-50
...
| ⏳ **RG-83** | **Carried over from 2026-09-02.** The updater endpoint still resolves to 404 — that is a *publishing* action (promote a full release), not a commit, and the fix pass instead made it **visible**: `npm run updater:check` reads the endpoint out of both configs and fetches it, and Settings → Updates now reports the channel rather than the absence of news (RG-92) |
...
| ⏳ RG-83 | **The auto-updater endpoint returns 404.** Both configs point at `…/releases/latest/download/latest.json`; GitHub's `/latest/` excludes pre-releases, and the only published release is a pre-release | **Every copy that ever ships is un-updatable.** The updater is built, signed with a real minisign key and covered by tests — and resolves to nothing. This class of failure is invisible from the source and green in every test, because the URL is data | `curl -sI …/releases/latest/download/latest.json` → **HTTP/2 404**, run 2026-09-02; `gh release list` shows one pre-release and four drafts | Publish a non-prerelease, or pin the endpoint at a tagged manifest. **Not a commit** — a publishing action | RG-73 | **P1** | S | The same `curl` returns 200 and a manifest whose `version` matches the tag **Instrumented from a third direction 2026-09-05**: `.github/workflows/updater-channel.yml` runs `node scripts/check-updater.mjs --strict` on demand, so the moment a full release is approved somebody can watch this go green from the Actions tab. Deliberately a button and not a gate — a step in `release.yml` would be red on every tag, because the release it checks is still a draft at that moment, and a permanently red check is one everybody learns to ignore. A weekly `schedule:` is worth adding the day this closes and pointless while it is open. |
