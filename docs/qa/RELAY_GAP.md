...
| ⏳ RG-137 | **A 28.6 second worst case on `audio_to_visible_transcript`, in a service where nothing was dropped and nothing panicked.** Service 16 final `perf_samples`: p50 2602 ms, p95 4694 ms, p99 5647 ms, **worst 28650 ms** over 4040 samples; `transcript_cadence` worst 20978 ms over 3936. The process log agrees independently — 4416 `LAG=` lines, median 1287 ms, p90 2628 ms, **max 20977 ms** — so this is two instruments, not a stamping artefact | By rule 31's own definition the clock starts at the OLDEST audio still waiting, so for 28 seconds the oldest unprocessed audio had been waiting nearly half a minute. **A reference spoken in that window reaches the wall half a minute late or never.** Nothing on the fire path is responsible: `transcript_to_reference_detection` is 40 ms at p50 and `reference_detection_to_fire` is 9 ms, and end-to-end microphone-to-projector was 1521 ms p50 over eight fires. The delay is entirely in STT | `audits/FIELD-2026-09-13.md` §4. **Zero dropped audio in 110.5 minutes** — no `note_dropped_audio`, so the bounded queues of rule 33 never shed, which means the backlog was absorbed inside the decode rather than at a queue | **Not established, and that is the entry.** The plausible reading is the music: during worship the log shows `drained=10` to `drained=16` per pass against `drained=1` during speech, at or past break-even, with decode costs of 2000-3100 ms. A **debug build** (`dev` profile, unoptimized) plus dense full-band audio explains it without anything being wrong — and is also exactly what a real backlog looks like. Replay the WAV through `stt::realtime::live_transcript_latency`, the one rig that runs at the speed of speech, in a RELEASE build, and compare | RG-118 for the cadence metric's own defect · RG-116 | **P1** | M | Replay the 110.5 minute recording in release; if the tail disappears the finding is the profile and this row closes as measured, if it survives the music is a real backlog and the cadence floor wants revisiting |
---
