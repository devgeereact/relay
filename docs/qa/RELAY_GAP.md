...
**112 entries. 103 closed, 1 withdrawn as wrong, 8 not closed** — RG-32 open, RG-41 and RG-50
flagged, **RG-83 and RG-85 carried over** from the 2026-09-02 audit, RG-98 and RG-101 recorded
as dependency debt, and **RG-108 filed and declined** for the RG-85 reason (it cannot be
verified without a kiosk screen). **RG-95, RG-96 and RG-97 were closed on 2026-09-04**, and
...
| ⏳ **RG-83 · RG-85** | **Carried over from 2026-09-02.** The updater endpoint still resolves to 404 — that is a *publishing* action (promote a full release), not a commit, and the fix pass instead made it **visible**: `npm run updater:check` reads the endpoint out of both configs and fetches it, and Settings → Updates now reports the channel rather than the absence of news (RG-92). The CSP's `http:`/`ws:` grants are still wider than the LAN they mean; narrowing them was **declined by this pass**, because `tauri dev` does not exercise the CSP and verifying a narrowing needs a packaged build driven through kiosk media playback, which no instrument here can do |
...
| ⏳ RG-85 | **The CSP grants `http:` and `ws:` to any host** in `img-src`, `media-src` and `connect-src`, where what it means is the LAN | The reason is real — the kiosk hub and the media server cannot be TLS on a LAN appliance — but the grant is written far wider than the need. `script-src`, `object-src`, `frame-src` and `form-action` are all tight | `tauri.conf.json` `app.security.csp`, read 2026-09-02. **A concrete path was found 2026-09-05**: `TemplateRender.svelte`'s `bgPaint` emits `url("${L.image}")` into a style attribute straight from template JSON, so a template that arrived by email **beacons an attacker-controlled `http://` URL the moment it is previewed** — the church's IP and the time it was opened, out of an app whose first promise is that nothing leaves the device. Svelte escapes the quotes, so there is no markup breakout, and `KIOSK_CSP` already blocks the same template on the LAN pages: the operator webview is the only affected surface | Scope the three to the LAN origins actually used. **`tauri dev` does not exercise the CSP at all**, so any change must be verified against `npm run tauri build` | T4 | P2 | M | A packaged build renders a kiosk source and a media background with the narrowed policy |
...
| ⏳ RG-108 | **The kiosk hub streams the content feed to any client that connects, before and without a `hello`, with no `Origin` check.** `tx.subscribe()` happens at accept and the loop forwards every broadcast; registration gates nothing | WebSockets are exempt from CORS, so a plain-`http:` page opened on a phone on the church wifi can `new WebSocket('ws://<relay>:8031')` and receive the feed — which carries not only the verse on the projector but `stage_note` and `next_reference`/`next_text`, the preacher's own monitor. DECISIONS §35 accepts that someone in the room can SEE the wall; this is content leaving the building, which `SECURITY.md` §1 ranks above everything else | Read of `channels::run_kiosk_server`, 2026-09-05 | Reject a handshake whose `Origin` is neither absent nor the `:8032` origin, via `accept_hdr_async`. **Declined by this pass for the RG-85 reason**: an OBS browser source sends the `:8032` origin and is fine, a church-built kiosk page on another origin is not, and no instrument here can drive a real kiosk screen to find out which exists in the wild | RG-85 | P2 | M | A packaged build serving a real kiosk screen and an OBS source with the check in place, and a `ws://` connection from another origin refused |
