...
| 37 | Accessibility mode (high visibility) | **PARTIAL** | RG-18. High Visibility is a **look a template carries**, not a parallel renderer — so it inherits the one template engine and cannot drift from it. It shipped as a THEME; themes were folded into templates (DECISIONS §87) and its style is frozen in `src-tauri/data/legacy_themes.json` and inlined into any template that pinned it, so a church that already chose it still has it, at the same 21:1 (`legibility.test.js`, which measures those frozen bytes and says so). Contrast is computed exactly over a solid background and reported as **"cannot be checked"** over a picture, never guessed. The console side was already served: `sr-only` live regions, `role="meter"`, `trapFocus`, reduced motion | **A church that did not already choose it has no high-contrast look on the shelf to pick.** The theme was the only way to select one, and deleting the theme desk removed the selection without replacing it; Track D seeds High Visibility as a template FAMILY and closes this. Was EXISTS and "Nothing in the product" until 2026-09-16 — the status did not move when the capability did, which is the shape this register exists to catch. Plus the same Stage B caveat as §36: the thresholds are unverified |
