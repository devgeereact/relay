...
| ⏳ RG-136 | **The service timeline stamped an `output_recovered` with no `output_lost` before it, and ended on a loss that never recovered.** `service_events` for service 16 is three rows entire: `service_started` at 0.0 s, `output_recovered` for **Streaming** at 3207296.6 ms, `output_lost` for Streaming at 3245372.7 ms. A screen came back from an outage the timeline never recorded | **The timeline understates the outage count by at least one, and it is the instrument the Sunday report and replay are both built on** (RG-04, RG-07, RG-08). Worse in the other direction: the operator's connected screen died at 3245.4 s of a 6631.7 s service and **was dead for the final 56.5 minutes**. Nothing put scripture on a wall in that window only because nothing was called after 3097.1 s — this is luck, not a guarantee. RG-119 recorded three loss/recovery pairs in 85.5 minutes and asked whether the screen stops painting or the heartbeat stops arriving; this adds a third question, why one transition produced no row at all | `audits/FIELD-2026-09-13.md` §3; `select * from service_events where service_id=16` returns exactly those three rows. The recovery's own detail string is *"Streaming · screen's own clock: silent 2s, never hidden"*, so the page went quiet while VISIBLE — not backgrounded by the OS, which is the case `OutputHealth` exists to catch, and it caught that half | Find why the first loss produced no event before changing anything: a missing row and a spurious row want opposite fixes, and RG-119 is still open on the same surface. Do not "fix" this by suppressing the unmatched recovery — an event with no pair is the evidence | RG-119 · RG-02 · RG-04 | **P1** | M | A screen killed mid-service must produce a matched `output_lost`/`output_recovered` pair; assert the timeline has no unpaired recovery across a full recorded service |
