...
**122 entries. 112 closed, 1 withdrawn as wrong, 9 not closed** — **RG-115 to RG-122 were filed on 2026-09-06 from a live service**, eight rows off one morning and three of them closed, which is the honest shape of a field audit: most of what a service finds cannot be settled by a commit, and the ones that can tend to be the instruments rather than the product. Three of the eight were **defects in the instruments** (RG-118 cadence counting silence, RG-120 an end-to-end stage that recorded 0 samples against three fires, RG-122 no record of which microphone a service used), which continues the pattern this register keeps recording about itself; RG-122 is now closed, and with it RG-121, the reason the record mattered — the selected input device was never persisted, so every launch silently reverted to the system default and a church could run a service on a laptop microphone with the desk feed plugged in. Both halves are fixed: the device is remembered, and a remembered device that is not attached today falls back **visibly** rather than silently. **RG-119 is the one to look at first**: the main output was lost and recovered three times in 85.5 minutes, 22.6% of the service, and nobody yet knows whether the screen stopped painting or the heartbeat stopped arriving. The original three, and only the first of the three is closed: a chapter the preacher stated out loud was answered from memory and put **Psalms 92:10** on a real output at 0.88, which is the second wrong verse in two field services and is fixed and held by a test that reproduces it when reverted. The two left open are the ones a commit cannot settle — the STT model turns out to be an **accuracy** trade the product offers as a speed setting (**5 of 9 auto-fires correct on `ggml-base` against 3 of 3 on `turbo`**, in the same service), and a runtime audio-stream error is one line on stderr that stops nothing and tells nobody. RG-32 open, RG-41 and RG-50
...
| ✅ RG-121 | **The audio input device is not persisted, so every launch silently reverts to the system default.** `capture.js:62` holds `inputDevice` in memory only (`inputDevice: ''`), and both `Live.svelte` and `Settings.svelte` pass `$capture.inputDevice \|\| null` into `startCapture`. Nothing writes it to `app_settings` or to `environment_profiles`, which is empty on this install | **This is the difference between the desk feed and the room**, and it is silent in both directions. On 2026-09-06 both launches started on `MacBook Pro Microphone` with a Blackmagic Web Presenter 4K plugged in and selected in the previous session. A volunteer who does not know to re-select it runs the service on a laptop microphone at the back of a booth, and rule 12's learned gate will do its best with it. Relay stores the learned room floor across sessions (RG-10) and does not store which device it learned it on | `capture.js:62`; both launch logs of 2026-09-06; `environment_profiles` empty; `audits/FIELD-2026-09-06.md` F-15 | **FIXED 2026-09-07.** The device is written to `app_settings` under `audio.input_device` whenever it is set, from the console, from Settings, or by applying a room, and `initAudio` restores it. **It degrades honestly, which was the harder half of the row.** `chooseInputDevice` is a pure function holding the whole rule: a remembered device that is attached is selected; one that is not here today is NOT selected and is reported instead, so Relay falls back to the system default and says which microphone it expected. Falling back silently would be this same bug with an extra step. The report is a `degraded.js` row, so it is on every tab rather than only on the Settings pane an operator has no reason to open: *"Blackmagic Web Presenter 4K is not plugged in ... Relay is listening on the computer's default input instead, which in a hall is usually much further from the preacher"*, at `reduced` and not `blocked`, because Relay genuinely is still listening. Two values that are not faults and raise nothing: `''`, meaning the operator chose the system default, and never having chosen. They are different facts and both are silent. Not moved into RG-10's room profile: a room is applied deliberately and this has to happen at launch, on a machine where nobody opens Settings. The room already carries `inputDevice` and now persists through the same setter | RG-10 for the profile it belongs in | **P1** | S | Reopen with a device selected and assert it is still selected; unplug it and assert the operator is told rather than silently moved to the default |
| ✅ RG-122 | **No log line names the audio input device, so no audit can establish which input a service used.** All that is printed is `audio: capture @ 48000 Hz · denoise on (RNNoise)`. Both candidate devices on 2026-09-06 run at 48 kHz, so nothing in the log, the database or the diagnostic bundle distinguishes them | Small, and it invalidates the most important caveat an audit can carry. `audits/FIELD-2026-09-06.md` states its input as the Blackmagic desk feed **on the operator's word alone**, because no instrument in the run recorded it. Any future comparison of detection accuracy between services is comparing two unknown microphones, and with RG-121 unfixed the device genuinely does change between launches without anybody choosing it | Read of `audio::build_and_run`'s startup prints, 2026-09-06; `audits/FIELD-2026-09-06.md` F-16 | **FIXED 2026-09-07.** The startup line now reads `audio: capture @ 48000 Hz · denoise on (RNNoise) · input "Blackmagic Web Presenter 4K" (chosen)`, and the diagnostic bundle carries the same phrase as a **Microphone** fact. **It says whether anybody chose it, not just which one it is**, because that is the half RG-121 needs: a launch that silently fell back to the system default reads identically to one an operator set up, and `(chosen)` against `(system default)` is the only thing that separates them. Recorded after `stream.play()` rather than at resolution, since a device that resolved and then would not start is not the microphone this service used. Before any capture opens, `last_input()` is **None** and the bundle says *not opened yet this run* rather than naming the default, which would be exactly the false confidence this row exists to remove. Content-free: a device name is hardware | RG-121 · RG-12 for the bundle | P3 | S | Start capture on a named device and assert the log and the bundle both name it |
---
