...
and by reading §23. Last updated 2026-09-03.*

**100 entries. 90 closed, 1 withdrawn as wrong, 9 not closed** — RG-32 open, RG-41 and RG-50
flagged, **RG-83 and RG-85 carried over** from the 2026-09-02 audit, and **RG-88 … RG-97 filed
2026-09-03 by the V1 production audit's fix pass** ([RELAY_V1_AUDIT.md](../RELAY_V1_AUDIT.md)),
seven of which it closed in the same pass. Of the eight not closed, **two are not code**: RG-83
is a publishing action and RG-73's certificates are a purchase.
...
| ⏳ **RG-95 … RG-98** | **Filed and deliberately left open 2026-09-03.** The error state missing from twelve more list surfaces (mechanical, and twelve mechanical edits is where a real regression hides), no HTTP `Range` on the media server, no rate limit on a control plane that is already open by decision, and a LAN-bound Vite dev server carrying two live advisories whose only fixes are semver majors || ❓ **§19b** | **Two open questions, neither a commit.** The translation one above, and — added 2026-08-31 — whether a detection should carry the audio quality it was heard in. The measurement exists (`dsp::AudioQuality`); this report previously said it did not, and that error is corrected in place |
...
| ⏳ RG-95 | **The ERROR third of Loading/Empty/Error exists on 3 of about 15 list surfaces.** `Channels`, `TemplateGallery` and (since RG-93) `History` read `readErrors`; `Announcements`, `Arrangements`, `MediaLibrary`, `Scripture`, `LyricsPane` and the rest still show an empty sentence over a failed read | Lower stakes than RG-93 — those tables are ones the operator filled, so "you have none" is at least *plausible* — but it is the same lie, and the machinery to fix it already exists and is free to use | Survey of `EmptyState` / `Loading` / `readErrors` usage per view, 2026-09-03 | Each list reads `$readErrors.<key>` and renders `ErrorState` before its empty sentence. Deliberately NOT swept in one pass: twelve files of mechanical edits is exactly where a real regression hides | RG-93 for the pattern | P2 | M | A `surface.test.js` case per view, rejecting the read and asserting `[role="alert"]` |
| ⏳ RG-96 | **`serve_media_file` answers no HTTP `Range` request.** It streams the whole file from byte zero, every time | A browser `<video>` on a kiosk or OBS source cannot seek, and cannot resume a dropped connection — it starts the clip again. Acceptable for a background loop that plays start to finish; not for anything an operator wants to scrub | Read of `channels::serve_media_file`, 2026-09-03 (the RG-90 streaming fix removed the memory cost, not this) | Parse `Range`, answer `206` with `Content-Range`, advertise `Accept-Ranges: bytes` | RG-90 | P2 | S | A `Range: bytes=` request returning 206 with the right slice, and a whole-file GET still returning 200 |
| ⏳ RG-97 | **The LAN control plane has no rate limit and no connection cap.** Every accepted socket spawns a task; `/api/search` runs a semantic scan and an FTS query per request | Not a new exposure — DECISIONS §35 already accepts that anyone on the church wifi can drive the wall — but an accidental loop in a kiosk page can saturate the laptop running the sermon, and nothing would say why. The read timeout (RG-90) bounds an idle connection, not a busy one | Read of `run_output_http_server`, 2026-09-03 | A concurrent-connection semaphore and a token bucket per route, sized so the preacher's phone never notices | — | P3 | S | A burst of N concurrent requests answered without the console stalling |
| ⏳ RG-98 | **The Vite dev server binds every interface, and `npm audit` reports a HIGH and a MODERATE against exactly that surface.** Ten vulnerabilities, **all ten in dev tooling** — production dependencies are clean (`npm audit --omit=dev` → 0) | Vite's path traversal in optimized-deps `.map` handling and esbuild's *"any website can send any request to the dev server and read the response"* are dev-server bugs, and `vite.config.js` puts this dev server on the LAN on purpose so an OBS machine can load the output page. A packaged Relay has no server on 5032 at all, so the exposure is exactly as long as somebody is running `npm run tauri dev` — which the documentation encourages doing at a church | `npm audit` and `npm audit --omit=dev`, 2026-09-03; `vite.config.js` `server.host` | Every fix is a semver **major** (vite 5 → 8, vitest 2 → 4, svelte 4 → 5) and none is an audit-pass edit — Svelte 4 is a recorded stack choice and a runes migration would touch every component. Recorded at the line instead, with the two advisories named, so the next person weighing `host: 'localhost'` has the facts rather than a preference | — | P2 | M | `npm audit --omit=dev` staying at 0, and a decision recorded in `DECISIONS.md` if the default is ever flipped |
