...
| ⏳ RG-120 | **`end_to_end_speech_to_scripture` stamped 0 samples in a service with three auto-fires.** Service 14: `reference_detection_to_fire` recorded 3 samples and the end-to-end stage recorded **none**. Service 15 recorded 7 against nine auto-fires. The stage is unreliable, not merely sparse | Rule 31 states that a stage never reached is an ABSENCE and not a zero, and this is the stage that answers the only question a church actually asks: **how long after the preacher says it does it appear.** An absence is at least honest, and 0 of 3 means the number cannot be quoted from a single service at all. The figures it did produce in service 15, **p50 2282 ms / p99 3855 ms**, are the first credible end-to-end numbers this project has and they do **not** reproduce F-6's 39 to 90 s. On a different model in a different service, so they do not close F-6 either | `perf_samples`, services 14 and 15, `audits/FIELD-2026-09-06.md` F-13 | **Investigated 2026-09-07, and the headline of this row was wrong.** The eviction theory does not hold, and neither does *"the stage is unreliable"* for service 14: **nothing was attached to paint those fires.** `service_events` records no output attachment in service 14 until the `output_lost`/`output_recovered` pair at 1457.7 s, which is the signature of a window being OPENED (RG-119: a never-answered attachment logged a loss on the first poll and a recovery on the next). All three auto-fires were at 181.9 s, 212.0 s and 460.4 s, before it. Service 15 opened a window at 4.9 s and reported 7 of 9. So **0 of 3 was the correct answer**, and rule 31's own rule applies to it: a stage never reached is an absence, not a zero. **The real defect is that the absence had no cause.** *"The AI never fired"* and *"nothing was attached to paint what it fired"* are the same number in the report, and they are completely different reports about a church. Two counters now separate them, both in Settings → Diagnostics and in the diagnostic bundle: **verses no screen reported painting** (counted in `retire`, the one place every trace stops being able to receive anything) and **render reports that arrived too late** (a mark for a trace already closed, expired or evicted — the instrument's own fault rather than the room's). **The test this row asked for exists and passes on today's code**: `e2e::one_auto_fire_that_reached_a_screen_is_one_end_to_end_sample` drives the real path — decode pass, transcript, detection, fire, then the screen's mark through the same command `latency.js` calls — and asserts exactly one sample. Its neighbour asserts three screens painting one verse is still one measurement, because the obvious fix here is to stop closing the trace on the first mark and it would silently triple every count in the report. **What is still unexplained: the two missing samples in service 15** (7 of 9). `perf_samples` stores percentiles, not traces, so the service that produced them cannot be re-read. The counters above are what make the next one answerable | RG-14 · F-6 in `audits/FIELD-2026-08-30.md` | P2 | M | A test that drives one auto-fire and asserts the end-to-end stage records exactly one sample. Nothing asserts that today, which is why 0 of 3 shipped unnoticed |
