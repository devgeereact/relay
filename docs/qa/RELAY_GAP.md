...
**87 entries. 78 closed, 1 withdrawn as wrong, 8 not closed** — RG-32 open, RG-41 and RG-50
flagged, and **RG-83 … RG-87 filed 2026-09-02 by the V1 production audit**
([RELAY_V1_AUDIT.md](../RELAY_V1_AUDIT.md)). Of the eight, **two are not code**: RG-83 is a
...
| ⏳ **RG-83 … RG-87** | **Filed 2026-09-02 by [RELAY_V1_AUDIT.md](../RELAY_V1_AUDIT.md).** A dead updater endpoint (verified with `curl`, HTTP 404 — a publishing action, not a commit), two unbounded audio queues described in the handbook as bounded, a CSP that grants `http:` to any host where it means the LAN, a rule stated absolutely and held approximately, and the absence of any instrument that checks a relative Markdown link — which is how twenty-eight of them broke in one afternoon |
