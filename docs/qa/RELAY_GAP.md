...
**112 entries. 105 closed, 1 withdrawn as wrong, 6 not closed** — RG-32 open, RG-41 and RG-50
flagged, **RG-83 carried over** from the 2026-09-02 audit, RG-98 and RG-101 recorded
as dependency debt. **RG-85 and RG-108 were filed as declined and then taken on the same day**,
...
| ⏳ **RG-98 · RG-101** | **The dependency rows, one per language.** `npm audit` is 10 advisories, all dev tooling, production clean — every fix a semver major, so it is recorded rather than taken. `cargo audit` had **never been run at all** until 2026-09-04: `h2` was fixed by a lockfile update, and two 7.5-HIGH `quick-xml` advisories reach Relay through `plist` through Tauri and need an upstream bump |
...
| ⏳ RG-98 | **The Vite dev server binds every interface, and `npm audit` reports advisories against exactly that surface.** Re-run 2026-09-04: **10 vulnerabilities, 1 critical, 1 high, 8 moderate — all ten in dev tooling**; production dependencies are clean (`npm audit --omit=dev` → 0) | Vite's path traversal in optimized-deps `.map` handling and esbuild's *"any website can send any request to the dev server and read the response"* are dev-server bugs, and `vite.config.js` puts this dev server on the LAN on purpose so an OBS machine can load the output page. The critical is vitest's **UI server** reading and executing an arbitrary file — this repository never starts it (`npm test` is `vitest run`; no `--ui` appears in `package.json` or any workflow), which is why it is recorded rather than treated as live. A packaged Relay has no server on 5032 at all, so the exposure lasts exactly as long as somebody is running `npm run tauri dev` — which the documentation encourages doing at a church | `npm audit` and `npm audit --omit=dev`, 2026-09-04; `vite.config.js` `server.host` | Every fix is a semver **major** (vite 5 → 8, vitest 2 → 4, svelte 4 → 5) and none is an audit-pass edit — Svelte 4 is a recorded stack choice and a runes migration would touch every component. Recorded at the line instead, with the advisories named, so the next person weighing `host: 'localhost'` has the facts rather than a preference | — | P2 | M | `npm audit --omit=dev` staying at 0, and a decision recorded in `DECISIONS.md` if the default is ever flipped |
...
| ⏳ RG-101 | **Nothing had ever checked Relay's RUST dependencies.** `cargo audit` was not installed, the launch checklist carried the row as ⬜ never checked, and no CI job runs it — so the half of the dependency tree that actually ships (the npm half does not: a packaged Relay contains no Node) was the unaudited half | Run 2026-09-04 after installing `cargo-audit`: **three advisories.** `h2 0.4.15` (unbounded empty DATA frames, reached through `reqwest` by the model download, the updater and Sentry) and **two 7.5-HIGH advisories against `quick-xml 0.39.4`** — unbounded namespace-declaration allocation, and quadratic time checking a start tag for duplicate attributes | `cargo audit`; `cargo tree -i quick-xml` and `cargo tree -i h2` for the paths | `h2` **fixed** by `cargo update -p h2` → 0.4.19, a lockfile-only change; the full Rust suite passes on it. `quick-xml` arrives through `plist` through **Tauri itself**, and the fix is `>=0.41`, which no lockfile edit can reach — it needs an upstream Tauri bump. Its exposure is narrow but not zero: the plists `plist` parses are Relay's own bundle files, not anything a church supplies. The 18 remaining warnings are unmaintained **GTK3 Linux bindings**, which neither shipped platform builds | — | P2 | S | `cargo audit` reporting only the `quick-xml` pair, and a CI job that runs it so the next advisory is not found by an audit eleven months later |
