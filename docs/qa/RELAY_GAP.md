...
| ✅ RG-95 | **The ERROR third of Loading/Empty/Error existed on 3 of about 15 list surfaces.** `Channels`, `TemplateGallery` and (since RG-93) `History` read `readErrors`; `Announcements`, `MediaLibrary`, `Scripture`, `LyricsPane`, `Browse`, the Planner's plan rail and the model chooser all showed an empty sentence over a failed read | Lower stakes than RG-93 in one sense — those tables are ones the operator filled, so *"you have none"* is at least plausible — and higher in another: every one of those sentences tells the operator to **do the work again**. *"No songs yet — import or paste one"*, *"No media yet — add some"*, *"No announcements yet — draft one"*, *"No plans yet"* on a Tuesday evening, and *"That chapter is empty"*, which is a claim about **scripture** rather than about a database. The model chooser was worse than a wrong sentence: it renders an `{#each}`, so a failed read drew a **blank panel** on the screen whose whole job is getting speech recognition working | Survey of `EmptyState` / `Loading` / `readErrors` usage per view, 2026-09-03; re-surveyed 2026-09-04 by comparing every `guardedRead` key against the keys any view reads — **2 of 29 keys had a surface** | Seven surfaces now read `$readErrors.<key>` and render `ErrorState` before their empty sentence, **and `chapterVerses` became a guarded read** — it swallowed into a bare `catch {}`, so the Bible pane had no key to read even if it had wanted one. Deliberately not swept in one pass: the remaining keys are diagnostics and pickers (`listMonitors`, `listTranslations`, `perfHistory`, `latency*`, `rooms`, `listVoiceProfiles`, `listTemplateVersions`, `loadContentTemplates`, `findModelFiles`, `languageReport` — since fixed under RG-110 — `serviceLock`, `verseRepeatCount`, `servicePerf`, `serviceTimeline`), none of which tells an operator to redo work. **Two surfaces have no key to read at all**: the Live cue rail and the Planner's cue table both render `planItems`, which swallows in a bare `catch {}`. Guard it the way `chapterVerses` was guarded — never by promoting it to a wrapper that throws, because `Live.svelte::loadPlan` has no `finally` and the run surface would be left saying *Loading cues…* for the rest of the service | RG-93 for the pattern | P2 | M | Seven `surface.test.js` cases, one per surface, each rejecting exactly that surface's read and asserting `[role="alert"]` and the absence of the sentence. **All seven verified by deleting the branch again and watching them fail** |
