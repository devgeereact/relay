...
**One difference from every previous field audit, stated because it changes what the evidence
is worth.** This is a **debug build**, not a packaged one. Decode costs below are therefore not
comparable with `PERF-MODELS-2026-08-30.md` or with either earlier field audit, both of which
ran packaged binaries. Accuracy results are not affected by the profile; latency results are,
and §4 says so in every row.
...
painting or the heartbeat stops arriving. **Today adds a third question it cannot answer: why
one of the two transitions produced no row at all.** Filed as **RG-136**.
...
**What caused it is not established.** The plausible reading is the music: during worship the
log shows `drained=10` to `drained=16` per pass (10 to 16 × 200 ms hops consumed per decode,
against `drained=1` during speech), which is at or past break-even, and decode costs of
2000-3100 ms against 8000 ms windows. A debug build plus dense full-band audio is enough to
explain it without anything being wrong. **It is also exactly what a real backlog looks like**,
and this audit cannot separate the two from the data it has. Filed as **RG-137**, with the
replay rig named in the row, because a 110.5 minute WAV of the audio that produced it now
exists and `stt::realtime::live_transcript_latency` is the one rig that runs at the speed of
speech.
