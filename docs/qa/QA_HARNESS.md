...
Re-measured **2026-09-14** against the merged tree (`rebrand/base` — `new_look_refresh` with `audit/field-2026-09-13` merged into it; both branches' own figures were measured on their own tree and neither survived the merge). Every number here is produced by a command,
...
| `.svelte` files | **52**, **25** of them views | `find src -name '*.svelte' \| wc -l`, and `find src/lib/views -name '*.svelte' \| wc -l` for the second — it read 22 with no reproducer beside it, and the views live in three directories |
| `<button>` occurrences | **367** | `grep -ro '<button' --include='*.svelte' src | wc -l` |
