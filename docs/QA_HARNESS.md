...
|---|---|---|
| Rust tests | **552 passing**, 28 ignored | `cd src-tauri && cargo test` |
| Frontend tests | **650 passing**, 0 skipped, 49 files | `npx vitest run` — read the runner's own summary line. **Not** `vitest list \| wc -l`: that stream carries Svelte compiler warnings too and over-counted by 7 |
| `e2e.rs` tests | **32** (30 run, 2 ignored — R2-C and R2-D, both open defects) | `cd src-tauri && cargo test e2e::` |
| Registered `#[tauri::command]` | **123** | `grep -c '#\[tauri::command\]' src-tauri/src/main.rs` |
| `.svelte` files | **47**, 22 of them views | `find src -name '*.svelte' | wc -l` |
| `<button>` occurrences | **322** | `grep -ro '<button' --include='*.svelte' src | wc -l` |
| Tables in the schema | **20** | `grep -c 'CREATE TABLE' docs/data/schema.sql` |
...
- **`song_arrangements` — no create path.** `save_arrangement` is registered, `saveArrangement`
  ...
  song arrangement at all, and CLAUDE.md's "every one of the 123 commands has a frontend
