...
| `output://channel_looks` | What each screen wears for each KIND of content — `{"1":{"scripture":9,"song":12}}` (DECISIONS §97). Ids only: the bytes ride as `template` frames once per connect, because one look carrying an embedded `data:` image has been 13 MB and `main::cue_or_content_tpl` refuses that cost on the fire path. Configuration, so it paints nothing on arrival; it changes what the NEXT fire of a kind wears. Sent on every hello including `{}`, and emitted here as well as published because a native output window has the bridge and no socket |
| `output://stage_alert` | A word to the preacher — `{ text }`, or `text: null` to take it down. **Emitted as well as published since 2026-09-21 (RG-156)**, because a screen wired as a native window and given the `stage` role heard nothing at all while the console reported a Stage Message sent. Neither door can address one screen — the hub records nothing about who connected (DECISIONS §35) and a Tauri emit is app-wide — so what keeps it off a congregation screen is `channelroles::acceptsStageMessage`, asked on the page at both doors from one function. The payload carries no field a congregation template binds |
| `output://channel_shows` | Which kinds each screen SHOWS at all — `{"1":["scripture","song"]}` (DECISIONS §98). ANDed with the template's own `layout.shows` at the receiver, so it can only NARROW: a screen can never force a template to paint a kind it has no regions for. An omitted screen is NO OPINION, which is not an empty list — the first shows everything its template does, the second shows nothing. **It is never consulted for `clear` or `black`**: a screen an operator could configure out of a panic control is rule 15's exact failure |
